...
export default function ProductDetail() {
    ...
    useEffect(() => {
        ...
        fetch('/api/products')
            ...
            const found = allProducts.find(p => p.id === parseInt(id as string));
...
if (!product) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-6">
          <button onClick={() => router.back()} className="mb-6 text-[#6F1D1B]">← Back to Shop</button>
      ...
      <div className="grid md:grid-cols-2 gap-12">
        <Image src={product.image_url} alt={product.name} width={600} height={600} className="rounded-2xl" />
        <div>
          <h1 className="text-4xl font-bold text-[#6F1D1B]">{product.name}</h1>
          <p className="text-3xl font-bold mt-4">${product.price}</p>
          <button onClick={() => addToCart(product)} className="mt-6 w-full bg-[#6F1D1B] text-white py-4 hover:bg-[#df716f] active:scale-95 rounded-xl">
            Add to Cart
                  </button>
      ...
      {reviews.map(review => (
        <div key={review.id} className="border p-6 rounded-xl mb-6 bg-white">
          <div className="flex justify-between">
            ...
            <button onClick={() => deleteReview(review.id)} className="text-red-600 text-sm">Delete</button>
          </div>
